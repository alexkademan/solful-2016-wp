...
module.exports = Backbone.View.extend({
    ...
    initialize: function () {
        ...
        app.mbClassSignInOut = new ClassSignInOut({model: this.model}); // catch AJAX calls t   join or cancel workouts
        ...
        app.mindBodyButton = new MindBodyButton({model: this.model}); // header banner butto    for front page
        app.mindBodyButtonFooter = new MindBodyButtonFooter({model: this.model}); // link to M  from footer
        ...
        if (this.$el.length === 1) {
            ...
            var wpSlug     = this.$("span.slug"),
                wpCategory = this.$("span.wpCat");

            // IF we have the MINDBODY URL and the page slug:
            if(
                this.model.get("mbFeedURL") !== false && (wpSlug.length > 0 &&
                wpSlug[0].innerHTML !== undefined && wpSlug[0].innerHTML !== "") &&
                (wpCategory.length > 0 && wpCategory[0].innerHTML !== undefined &&
                wpCategory[0].innerHTML !== "")
            ) {
                ...
                if(this.model.get("wpCategory") === "trainers"){
                    ...
                    /*
                     * we"re on the trainers page, so I left some JSON data
                     * there that I need...
                     */
                ...
                this.model.get("wpCategory") === "trainers") {
  ...
  makeAJAXcall: function( file, section ) {
      ...
      if( this.model.get("mbFeedUseSSL") === false ){
            console.log(thisURL);
      }
    ...
    ajaxDone: function(data, section) {
      ...
      switch(section) {
        ...
        case 'clientSchedule':
            app.mbLogInView.addRegisteredClasses(
                data.GetClientScheduleResult.Visits.Visit
            );
            break;
  ...
  // ************************
    ...
    renderAnotherTrainer: function(trainer) {
        trainer = new TrainerView({model: trainer});
        ...
        if( trainer.model.get('workoutCount') > 0 ){
            ...
            app.mbDays.each(function(day){
                ...
                if(classCount > 0) {
                    ...
                    /*
                     * show this day. Because there are workouts with the
                     * trainer in question.
                     */
                    var todayView = new DayView({
                        model: app.mbDays.get( today )
                    });
                    trainer.$('ul.workouts').append(
                        todayView.renderDay(trainerName, 'trainers').el
                    );
                }
            });
        }
    ...
    setBaseURL: function(){
        ...
        var baseMBlink = this.model.get('signupURLbase'),
            studioID   = this.model.get('studioID'),
